...
@implementation WCSenderIdCardCell

+(instancetype)senderIdCardCellWithTableView:(UITableView *)tableView
{
    static NSString *cellID = @"senderCellID";
    WCSenderIdCardCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if(!cell) {
        cell = [[WCSenderIdCardCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
        [cell setupUI];
        // 必须刷新布局，否则有些控件显示异常
        [cell layoutSubviews];
    }
...
- (void)setupUI {
    ...
    [_iconIV mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.mas_equalTo(self).offset(-ScaleW(10));
    ...
    [_bgIV mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.mas_equalTo(_iconIV.mas_left).offset(-ScaleW(10));
...
-(UIImageView *)bgIV {
    if(!_bgIV) {
        ...
        [_bgIV setImage:[UIImage imageNamed:@"chat_sender_bg"]];
    }
    return _bgIV;
}
